...
export default function ObjektaktePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  async function createDocument(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      await create({ action: 'objectDocument', objectId: id, type, serviceKey: values.serviceKey, notes: values.notes, title: `${typeTitle} - ${object.name}` });
  ...
  return <>
    ...
    {showDocument ? <AdminPanel title="Objektdokument anlegen" description="Die passende fachliche Vorlage wird mit dem aktuellen Objekt- und Kundenstand als unveränderlicher Entwurf gespeichert."><form className="admin-form" onSubmit={createDocument}><div className="admin-form-grid"><div className="admin-field"><label>Dokumenttyp</label><select name="type" required>{objectDocumentTypes.map(type => <option value={type.key} key={type.key}>{type.title}</option>)}</select></div><div className="admin-field"><label>Dienstleistung *</label><select name="serviceKey" required><option value="">Bitte wählen</option>{serviceCatalog.map(service => <option value={service.key} key={service.key}>{service.title}</option>)}</select></div></div><div className="admin-field"><label>Objektbezogene Ergänzungen</label><textarea name="notes" placeholder="Zum Beispiel konkrete Bereiche, Intervalle, ausgeschlossene Leistungen oder Feststellungen" /></div><button className="admin-button">Entwurf mit Fachvorlage anlegen</button></form></AdminPanel> : null}
